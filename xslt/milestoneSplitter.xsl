...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.tei-c.org/ns/1.0"
  ...
  <!-- v1.2 determines pagenumber of insertion -->
  <xsl:template name="getPatchPage">
    <xsl:param name="recursive" select="false()"/>
    <xsl:param name="create_element" select="true()"/>
    <xsl:choose>
      <xsl:when test="self::ptr[@type='displacement']">
        <xsl:variable name="targetPage">
          <xsl:sequence select="//floatingText[@xml:id=substring-after(current()/@target, '#')]/preceding::*[name()=$splitEl][1]"/>
          <xsl:if test="//floatingText[@xml:id=substring-after(current()/@target, '#')]/preceding::*[name()=$splitEl][1][not(@n)]">
            <xsl:element name="n" namespace="http://www.tei-c.org/ns/1.0"><xsl:number select="//floatingText[@xml:id=substring-after(current()/@target, '#')]/preceding::*[name()=$splitEl][1]" level="any"/></xsl:element> 
          </xsl:if>
          &#160;
        </xsl:variable>
        <xsl:variable name="targetBooklet">
          <xsl:sequence select="//floatingText[@xml:id=substring-after(current()/@target, '#')]/preceding::*[name()=$splitEl][1]/preceding::au:fb[1]"/>
          &#160;
        </xsl:variable>
        ...
        <xsl:if test="$debug">
          <xsl:comment>
            ...
            mode="unknown" 
            section="unknown" 
            template="getPatchPage" 
            id="GPP1"
          </xsl:comment>
        </xsl:if>
        
       <xsl:element inherit-namespaces="yes" name="ptr" namespace="http://www.tei-c.org/ns/1.0">
         <xsl:sequence select="@* except @target"/>
         <xsl:attribute name="target">
           <xsl:if test="$targetBooklet[*]">
             <xsl:text>b</xsl:text>
             <xsl:value-of select="$targetBooklet/au:fb/@n"/>
             <xsl:text>-</xsl:text>
           </xsl:if>
          <xsl:choose>
            <xsl:when test="//floatingText[@xml:id=substring-after(current()/@target, '#')]/preceding::*[name()=$splitEl][1][@n][not(preceding::au:fb)]">
              <xsl:choose>
                <xsl:when test="$targetPage/*[name()=$splitEl][@n]">
                  <xsl:value-of select="//floatingText[@xml:id=substring-after(current()/@target, '#')]/preceding::*[name()=$splitEl][1]/@n"/>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of select="$targetPage//n"></xsl:value-of>
                </xsl:otherwise>
              </xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:choose>
                ...
                <xsl:otherwise>
                  <xsl:value-of select="$targetPage//n"/>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:otherwise>
          </xsl:choose>
      ...
      <xsl:when test="self::ptr[@type='insertion']">
        <xsl:if test="$debug">
          <xsl:comment>
            CREATED
            mode="unknown" 
        ...
        <xsl:element name="floatingText" inherit-namespaces="yes" namespace="http://www.tei-c.org/ns/1.0">
          <xsl:sequence select="//floatingText[@xml:id=substring-after(current()/@target, '#')]/@* except @type"></xsl:sequence>
          <xsl:attribute name="type"><xsl:text>insertion</xsl:text></xsl:attribute>
          <xsl:for-each select="//floatingText[@xml:id=substring-after(current()/@target, '#')]/node()">
            <xsl:if test="$debug">
              <xsl:comment>
                SEQUENCED 
                mode="unknown" 
      ...
      <xsl:when test="self::floatingText[@type='displaced']">
        <xsl:variable name="correspPage">
          <xsl:sequence select="//ptr[@xml:id=substring-after(current()/@corresp, '#')]/preceding::*[name()=$splitEl][1]"/>
          <xsl:if test="//ptr[@xml:id=substring-after(current()/@corresp, '#')]/preceding::*[name()=$splitEl][1][not(@n)]">
            <xsl:if test="$debug">
              <xsl:comment>
                CREATED
                mode="unknown" 
            ...
            <xsl:element name="n" namespace="http://www.tei-c.org/ns/1.0">
              <xsl:number select="//ptr[@xml:id=substring-after(current()/@corresp, '#')]/preceding::*[name()=$splitEl][1]" level="any"/>
            </xsl:element> 
          </xsl:if>
        </xsl:variable>
        <xsl:variable name="correspBooklet">
          <xsl:sequence select="//ptr[@xml:id=substring-after(current()/@corresp, '#')]/preceding::*[name()=$splitEl][1]/preceding::au:fb[1]"/>&#160;
        </xsl:variable>
        <xsl:choose>
          <xsl:when test="$create_element">
            <xsl:if test="$debug">
              <xsl:comment>
                CREATED
                mode="unknown" 
            ...
            <xsl:element inherit-namespaces="yes" name="floatingText" namespace="http://www.tei-c.org/ns/1.0">
              <xsl:sequence select="@* except @corresp"/>
              <xsl:attribute name="corresp">
                <xsl:if test="$correspBooklet[*]">
                  <xsl:text>b</xsl:text>
                  <xsl:value-of select="$correspBooklet/au:fb/@n"/>
                  <xsl:text>-</xsl:text>
                </xsl:if>
                <xsl:choose>
                  <xsl:when test="//ptr[@xml:id=substring-after(current()/@corresp, '#')]/preceding::*[name()=$splitEl][1][@n][not(preceding::au:fb)]">
                    <xsl:value-of select="$correspPage/*[name()=$splitEl]/@n"/>
                    <xsl:value-of select="@corresp"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:choose>
                      <xsl:when test="$correspPage/*[name()=$splitEl][@n]">
                        <xsl:value-of select="//ptr[@xml:id=substring-after(current()/@corresp, '#')]/preceding::*[name()=$splitEl][1]/@n"/>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:value-of select="$correspPage//n"/>
                      </xsl:otherwise>
                    </xsl:choose>
                  </xsl:otherwise>
                </xsl:choose>
      ...
      <xsl:when test="self::floatingText[substring-after(@corresp, '#')=//ptr[@type='insertion']/@xml:id]"/>
      <xsl:otherwise>
        <xsl:choose>
  ...
  <xsl:template match="TEI" mode="#all" priority="1">
...
<!--        <xsl:sequence select="$milPos"/>-->
 ...
 <!-- <xsl:template match="xi:include" mode="#all">
    <xsl:choose>
      <xsl:when test="contains(@href, 'hands.xml')">
        <xsl:sequence select="//profileDesc"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:sequence select="//encodingDesc"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>-->
  ...
  <!-- 
    *****************
    COMMON ANCESTORS
    *****************
    Creates all ancestor elements common to alpha and beta  
  -->

  <xsl:template
    match="*[descendant::*[name()=$splitEl][@xml:id = $pageId] and descendant::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][preceding::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][number($pgRange)][@xml:id = $pageId]]]"
    mode="related">
  ...
  ****************
  ...
  <xsl:template
    match="*[descendant::*[name()=$splitEl][@xml:id = $pageId]  and not(descendant::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][preceding::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][number($pgRange)][@xml:id = $pageId]])]"
    mode="unrelated">

    <!--<xsl:choose>
      <xsl:when test="not(//TEI//*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][preceding::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][number($pgRange)][@xml:id = $pageId]])">
        <xsl:apply-templates select="/" mode="lastPage"/>
      </xsl:when>
    </xsl:choose>-->
  ...
  <!-- If contains text and parent contains text -->
  <!-- If contains 0 then there is text -->
  <xsl:template
    match="*[descendant::*[name()=$splitEl][@xml:id = $pageId]]"
    mode="lastPage">
  ...
  <xsl:template match="*[name()=$splitEl][@xml:id = $pageId]" mode="#all">
  ...
  <xsl:template
    ...
    <xsl:for-each select=".">
      <!--<xsl:if test="self::text()">a</xsl:if>-->
  ...
  ****************
  ...
  <xsl:template
    match="*[not(descendant::*[name()=$splitEl][@xml:id = $pageId]) and descendant::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][preceding::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][number($pgRange)][@xml:id = $pageId]]]"
    mode="unrelated">
  ...
  <xsl:template
    match="*[not(descendant::*[name()=$splitEl][@xml:id = $pageId])]
  ...
  <xsl:template
    match="*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][preceding::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][number($pgRange)][@xml:id = $pageId]]"
    mode="unrelated">
  ...
  <!-- Do not include beta -->
  <!-- <xsl:template match="*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][preceding::*[name()=$splitEl][not(@rend) or @rend != 'overstrike'][number($pgRange)][@xml:id = $pageId]]" mode="#all">
    <xsl:sequence select="."/>
  </xsl:template>-->
